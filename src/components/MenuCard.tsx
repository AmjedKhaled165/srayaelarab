      ...
      >
        {item.imageUrl ? (
          <button
            ...
            className="relative w-full h-40 sm:h-48 overflow-hidden cursor-pointer text-left"
          >
            <Image
              src={item.imageUrl}
              alt={item.name}
            ...
            {!item.available && (
              <div className="absolute top-3 right-3 px-2.5 py-1 bg-muted/80 backdrop-blur-sm text-background text-xs font-semibold rounded-full pointer-events-none">
                Out of Stock
              </div>
            )}
          </button>
